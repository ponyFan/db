...
public class JwtTokenUtils {
    ...
    // 创建token
    public static String createToken(Integer id, String username, Integer role, Integer group, boolean isRememberMe) {
        long expiration = isRememberMe ? EXPIRATION_REMEMBER : EXPIRATION;
        HashMap<String, Object> map = new HashMap<>();
        map.put(ROLE_CLAIMS, String.valueOf(role));
        return Jwts.builder()
                .signWith(SignatureAlgorithm.HS512, SECRET)
                .setClaims(map)
                .setIssuer(ISS)
                .setSubject(id + SPLIT_COMMA + username + SPLIT_COMMA + role + SPLIT_COMMA + group)
    ...
    // 从token中获取用户组
    public static Integer getGroup(String token) {
        List<String> userInfo = Arrays.asList(getTokenBody(token).getSubject().split(SPLIT_COMMA));
        return Integer.valueOf(userInfo.get(3));
    }

    // 从token中获取角色
    public static Integer getRole(String token) {
        List<String> userInfo = Arrays.asList(getTokenBody(token).getSubject().split(SPLIT_COMMA));
        return Integer.valueOf(userInfo.get(2));
    }
